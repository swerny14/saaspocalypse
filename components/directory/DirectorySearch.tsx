    ...
    >
      <div className="flex items-center px-[22px]">
        <input
          ...
          className="flex-1 py-5 font-display text-[22px] font-medium tracking-[-0.01em] bg-transparent outline-none border-none text-ink placeholder:opacity-50"
        />
      </div>
      <label className="flex items-center px-[18px] border-l-[2.5px] border-ink font-mono text-[11px] font-bold tracking-[0.1em] uppercase gap-2 max-[720px]:border-l-0 max-[720px]:border-t-[2.5px]">
        <span className="opacity-50">sort:</span>
        <select
          defaultValue={initialSort}
          onChange={onSortChange}
          aria-label="Sort scans"
          className="font-mono text-[11px] font-bold tracking-[0.1em] uppercase bg-transparent border-none outline-none cursor-pointer pr-1"
        >
          {sortOptions.map((opt) => (
