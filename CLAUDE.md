...
- `lib/seo/meta.ts` — title/description builders. `reportTitle` produces `"Can I build {name}? — {tier} · {time_estimate} · saaspocalypse"`; `reportOgTitle` is a longer Twitter/Slack-friendly form; `reportDescription` blends score + time + stack + first sentence of `take`, with graceful fallback when ≤155 chars overflows. `comparePageTitle` / `comparePageOgTitle` / `comparePageDescription` / `compareCanonical` produce the head-to-head metadata (`Can I build {a} or {b}?` form). **All metadata, OG, and JSON-LD `headline` fields call these so they don't drift.**
...
- **Moat scoring (Phase B):** `lib/normalization/moat.ts::scoreMoat({ components, capabilities, attributes })` — pure deterministic function over the projection. Six axes 0–10: `capital` (from `monthly_floor_usd` bucket + `is_usage_based` boost), `technical` (sum of `5 - commoditization_level` across linked components, scaled by `MOAT_RUBRIC_V1.technical_depth_cap`), `network` / `switching` / `data_moat` / `regulatory` (presence-counted from capability `moat_tags`, 4-per-hit curve, capped at 10). `aggregate` is a weighted RMS — `Math.sqrt(weightedSquares / totalWeight)` — so specialist moats (Stripe = capital + technical + regulatory only) don't get averaged down by zero-axes; weights live in `MOAT_RUBRIC_V1` (equal weights for V1). Bumping `RUBRIC_VERSION` keys the recompute script to rebuild every row. Persisted to `report_moat_scores` and joined into `StoredReport` as `moat: StoredMoatScore | null` via a Supabase 1:1 relational query (`*, moat:report_moat_scores(...)`); `normalizeMoatShape` defends against PostgREST occasionally returning the embed as a single-element array. Surfaced in `VerdictReport.tsx` via the `MoatBreakdown` component — typographic-hero design (giant aggregate numeral + 2×3 axis grid) with a severity-colored bar ladder: `coral` for fortress (≥7), `ink` for meaningful (≥4), `muted` for shallow (≥1), faded for none. The component takes a `slug` prop so the `methodology →` link in its header carries a `?from=/r/<slug>` round-trip query — the methodology page whitelists the path against `/^\/r\/[a-z0-9][a-z0-9-]{0,128}$/` and falls back to "back home" otherwise. **Distribution / brand are intentionally NOT scored** — you can't derive them from a homepage scan, and faking it would weaken credibility on the rest. The methodology page calls this out.

## Similarity engine

Powers the "the field." rail at the bottom of every `/r/[slug]`. Pure deterministic over the projection — no LLM at query time. Layered on top of the normalization layer's `report_capabilities` rows.
...
- **Verdict engine:** `lib/normalization/compare_verdict.ts::computeCompareVerdict(pair, diff)` is pure. Picks a winner (higher score wins; same-tier-and-`|delta| < 5` is a tie), assembles a chip (`BUILD <name> FIRST` / `TOO CLOSE TO CALL`), a one-line summary built from concrete reasons (tier delta → `a full tier easier`; cost delta over a 1.5× ratio → `smaller monthly bill`; moat aggregate → `matched moat depth` / `shallower moat`; stack count → `smaller stack`), a verdict-band line that wraps a tier-aware punch phrase (`build the X clone this weekend.` / `give the X clone a month.` / DON'T-tier hedge), and a CTA href to the winner's report. No LLM, no DB. Drives both the title-block sub-row and the bottom verdict band so they stay in lockstep.
- **SEO:** `comparePageTitle` produces `{a} vs {b}: which is easier to build? — saaspocalypse` — comparison intent (NOT the per-report "Can I build" intent). `comparePageJsonLd` emits a `WebPage` with `mainEntity = ItemList` of two `Review`s, one per side. `comparePageOgTitle` gets a longer share-card form including both scores. The OG image (`app/compare/[pair]/opengraph-image.tsx`) is a split panel — each side gets its tier-colored score + name + time, divider in the middle shows `vs` and the signed score delta. Same Satori sanitization rules as the per-report OG.
...
- **Landing:** `/admin` is a list of curation surface cards (unknowns, moat anomalies, similarity gaps), each linking to its respective queue. Replaces the previous behavior of auto-redirecting to `/admin/unknowns`. Add new surfaces by appending to `SURFACES` in `app/admin/page.tsx`.
...
- **Moat anomalies review:** `/admin/moat-anomalies` is the ops surface for fixing moat-scoring coverage gaps. It runs `lib/normalization/moat_anomalies.ts::detectAnomalies` over every scored report and flags ones whose moat aggregate doesn't fit their buildability tier — DON'T-tier with aggregate < 5, WEEKEND-tier with aggregate > 5, isolated zero-axis amid neighbors ≥ 7, DON'T-tier with no axis ≥ 7. The page surfaces ALL scored reports (not just heuristic-flagged) under three mutually-exclusive queue tabs: **Pending review** (default — anomaly + non-anomaly rows that haven't been verified), **Verified** (dismissed rows), **All scored** (escape hatch). Per-row: shows axis breakdown + which capabilities fired + verdict-text excerpt; offers (a) "Add pattern to existing capability" (most common fix — when a real report uses phrasing the patterns don't catch), (b) "+ New capability" with full form (when the gap is structural), (c) "✦ AI audit this report" — calls `lib/normalization/moat_audit_llm.ts::auditMoat` (Sonnet 4.6, ephemerally-cached prompt that includes the full capability catalog + currently-firing capabilities) which proposes specific `add_pattern` / `new_capability` edits with one-click "Apply" buttons, (d) "↻ Recompute this report" — single-report rebuild against DB-fresh taxonomy, and (e) "✓ Mark verified" — sticky dismissal that moves the row to the Verified queue (re-flag any time). Top of page has "Recompute all reports →" for the same operation across the corpus. Verified state lives on `report_moat_scores.review_status` ('pending' | 'verified') and survives recomputes — re-flag manually after a material taxonomy change if the dismissal might be stale. **Same posture as the unknowns flow:** admin-only, deterministic engine unchanged, LLM is curation aid only.
...
- **Live-taxonomy recompute (important):** The admin recompute paths read taxonomy *from the DB*, not from the TS-bundled defaults that production runScan uses. `lib/db/taxonomy_loader.ts::loadEngineContextFromDb` joins all four taxonomy tables and assembles a fresh `EngineContext` (via `engine.ts::buildEngineContext`) which is passed to `projectReport(ctx)` and `scoreMoat({ catalog })`. This means: admin adds a pattern via UI → DB updated → click recompute → new score reflects the pattern immediately, no redeploy needed. Production scans continue to use `DEFAULT_ENGINE_CONTEXT` (built once at module load from the TS imports) so deploys remain reproducible from git. To roll admin-side edits forward into production: run `pnpm tsx scripts/dump_taxonomy.ts` → review the diff → commit → deploy.
...
2. ~~**"Products like X" rail at bottom of every `/r/[slug]`**~~ — **shipped.** See "Similarity engine" above for the IDF² + descriptor-boost calibration, the comparison-framed cards (score delta + wedge), and the `/admin/similarity-gaps` curation flow.
3. **"How to attack X" wedge inversion** — surface a report's weakest moat axes as competitive wedges ("low switching cost + no data moat → undercut on portability"). Slots next to the verdict on DON'T-tier reports where the buildability angle dies. Pure projection over `report_moat_scores`, no LLM.
