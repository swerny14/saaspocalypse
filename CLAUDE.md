...
`/admin/*` routes are cookie-gated by `ADMIN_SECRET` (single shared secret, no multi-user accounts). Today only `/admin/unknowns` exists; future curation tools (component editor, report moderation) will follow the same auth pattern.
